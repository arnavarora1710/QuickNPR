...
void GSScene::load(const std::shared_ptr<VulkanContext>&context) {
    ...
    auto* vertices = static_cast<Vertex *>(vertexStagingBuffer->allocation_info.pMappedData);
    ...
    for (auto i = 0; i < header.numVertices; i++) {
        ...
        // vertices[i].normal = glm::vec4(vertexStorage.normal, 0.0f);
        ...
        // memcpy(vertices[i].shs, vertexStorage.shs, 48 * sizeof(float));
