...
inline void setClearBit(uint32_t &num, uint32_t n, bool value) {
    num = value ? num | (1U << n) : num & ~(1U << n);
}

void Renderer::handleInput() {
    auto translation = window->getCursorTranslationPosition();
    auto keys = window->getKeys(); // W, A, S, D
    auto mouse = window->getMouseButton();
    mouseX = translation[2];
    mouseY = translation[3];

    for (int i = 0; i < mouse.size(); i++)
        setClearBit(mouseFlags, i, mouse[i]);


    if ((!configuration.enableGui || (!guiManager.wantCaptureMouse() && !guiManager.mouseCapture)) && mouse[0]) {
    ...
    if (!configuration.enableGui || guiManager.mouseCapture) {
        if (translation[0] != 0.0f || translation[1] != 0.0f) {
            ...
            yaw += static_cast<float>(translation[0]) * 0.005f;
            pitch -= static_cast<float>(translation[1]) * 0.005f;

            // Clamp pitch to avoid flipping
            const float minPitch = glm::radians(91.0f);
            const float maxPitch = glm::radians(269.0f);
...
void Renderer::createRenderPipeline() {
    ...
    renderUniformBuffer = Buffer::uniform(context, sizeof(RenderUniformBuffer));
    ...
    inputSet->bindBufferToDescriptorSet(2, vk::DescriptorType::eStorageBuffer, vk::ShaderStageFlagBits::eCompute,
                                        sortVBufferEven);
    inputSet->bindBufferToDescriptorSet(3, vk::DescriptorType::eUniformBuffer, vk::ShaderStageFlagBits::eCompute,
                                        renderUniformBuffer);
    // inputSet->bindBufferToDescriptorSet(2, vk::DescriptorType::eStorageBuffer, vk::ShaderStageFlagBits::eCompute,
    //                                     sortKBufferOdd);
    inputSet->build();
...
void Renderer::updateUniforms() {
    ...
    RenderUniformBuffer renderData{};
    renderData.mouseX = mouseX;
    renderData.mouseY = mouseY;
    renderData.flags = mouseFlags | guiManager.brushes << 4;
    renderData.brushRadius = guiManager.brushRadius;
    renderData.brushStrength = guiManager.brushStrength;
    renderUniformBuffer->upload(&renderData, sizeof(RenderUniformBuffer), 0);
}
